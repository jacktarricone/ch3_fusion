...
p1_stack <-rast(list.files("./new_dswe/p1", full.names = T))
names(p1_stack) <-c("FLM","IMS","Landsat","MODIS","MODSCAG","VIIRS")
p2_stack <-rast(list.files("./new_dswe/p2", full.names = T))
names(p2_stack) <-c("FLM","IMS","Landsat","MODIS","MODSCAG","VIIRS")
p3_stack <-rast(list.files("./new_dswe/p3", full.names = T))
names(p3_stack) <-c("FLM","IMS","Landsat","MODIS","MODSCAG","VIIRS")
p4_stack <-rast(list.files("./new_dswe/p4", full.names = T))
...
ggsave(p,
       file = "~/ch3_fusion/plots/dswe_plot_v2.png",
       width = 7, 
       height = 12,
       dpi = 300)

system("open ~/ch3_fusion/plots/dswe_plot_v2.png") 
